...
char validatingTask(string inFile,string outFile,string juriFile,string testProgram="");
...
{
     addLog("Cheking task number "+to_str(task.taskNumber+1)+". Participant "+participantName);
     if (skipCheckedPrograms)
     {
       int newTime=getFileTime(task.path.c_str());
     ...
     task.score=0;
     for(int i=0;i<tsk.tests.size();i++){
         CopyFile(tsk.tests[i].testFile.c_str(),(runDir+tsk.inFile).c_str(),0);
         addLog(participantName+": task "+to_str(task.taskNumber+1)+" test "+to_str(i+1)+" ",0);
         ...
         if (!tsk.checkProgram.empty())
           {
              ...
              res=validatingTask(tsk.inFile,tsk.outFile,"judge.sol1","check.exe");
           } else res=validatingTask(tsk.inFile,tsk.outFile,"judge.sol1");
         task.result+=res;
         if (res=='+') task.score+=tsk.tests[i].maxScore,addLog("Right answer!!! "); else
         if (res=='P') task.score+=tsk.tests[i].maxScore*tsk.wrongFormat,addLog("Presentation error.");else
         addLog("Wrong answer...");
         clearDir(runDir,compiler.outFile);
     }
     addLog("Task "+task.path+" Result: "+task.result+" score "+to_str(task.score));
     addLog("***********************************************************************");
}
void checkAll()
{
     ...
     for (i=participants.begin();i!=participants.end();i++)
         for (int j=0;j<i->second.size();j++)
                  f<<i->second[j].path<<endl<<i->second[j].result<<endl<<
                       i->second[j].score<<endl<<getFileTime(i->second[j].path.c_str())<<endl;
     f.close();
}
