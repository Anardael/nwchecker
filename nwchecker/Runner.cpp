...
unsigned long int runProgram(string name, int verbose=0)
{
	...
	if (verbose)
		addLog("Running "+name+" Time took: "+to_str(GetTickCount()-timer)
				+" Return status: "+to_str(res));
	return res;
}

char runProgramTimer(string name, unsigned int time, int verbose)
{
	...
	do
	{
		GetProcessTimes(pinf.hProcess, &crt, &ext, &krnl, &usr);
		Sleep(10);
	} while ((ext.dwLowDateTime==0)&&(usr.dwLowDateTime/10000<=time)
			&&(GetTickCount()-timer<time*2));
	if (GetTickCount()-timer>=time*2)
	{
		TerminateProcess(pinf.hProcess, 0);
		if (verbose)
			addLog("W||smthing bad");
		return 'W';
	}
	else if (ext.dwLowDateTime==0)
	{
		TerminateProcess(pinf.hProcess, 0);
		if (verbose)
			addLog("TLE. Time: "+to_str(usr.dwLowDateTime/10000));
		return 'T';
	}
	else
	{
		...
		if (res==STILL_ACTIVE)
		{
			TerminateProcess(pinf.hProcess, 0);
			if (verbose)
				addLog("TLE (2). Time: "+to_str(usr.dwLowDateTime/10000));
			return 'T';
		}
		else if (res!=0)
		{
			if (verbose)
				addLog("RE "+to_str(res)+" Time: "+to_str(usr.dwLowDateTime
						/10000));
			return 'R';
		}
	}
	if (verbose)
		addLog("OK. Time: "+to_str(usr.dwLowDateTime/10000)+" ", 0);
	return '+';
