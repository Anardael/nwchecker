 ...
 */
 ...
 (function($) {
	...
	var DAYS_OF_WEEK_JA = ['æ—¥', 'æœˆ', 'ç�«', 'æ°´', 'æœ¨', 'é‡‘', 'åœŸ'];
	var DAYS_OF_WEEK_RU = ['Ð’Ñ�', 'ÐŸÐ½', 'Ð’Ñ‚', 'Ð¡Ñ€', 'Ð§Ñ‚', 'ÐŸÑ‚', 'Ð¡Ð±'];
	var DAYS_OF_WEEK_BR = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'SÃ¡b'];
	var DAYS_OF_WEEK_PT = ['dom', 'seg', 'ter', 'qua', 'qui', 'sex', 'sÃ¡b'];
	var DAYS_OF_WEEK_CN = ['æ—¥', 'ä¸€', 'äºŒ', 'ä¸‰', 'å››', 'äº”', 'å…­'];
	var DAYS_OF_WEEK_DE = ['So', 'Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa'];
	var DAYS_OF_WEEK_SV = ['SÃ¶', 'MÃ¥', 'Ti', 'On', 'To', 'Fr', 'LÃ¶'];
	var DAYS_OF_WEEK_ID = ['Min','Sen','Sel', 'Rab', 'Kam', 'Jum', 'Sab'];
	var DAYS_OF_WEEK_TR = ['Pz', 'Pzt', 'Sal', 'Ã‡ar', 'Per', 'Cu', 'Cts'];
	var DAYS_OF_WEEK_ES = ['dom', 'lun', 'mar', 'miÃ©r', 'jue', 'viÃ©', 'sÃ¡b'];
	var DAYS_OF_WEEK_KO = ['ì�¼', 'ì›”', 'í™”', 'ìˆ˜', 'ëª©', 'ê¸ˆ', 'í† '];
	var DAYS_OF_WEEK_NL = ['zo', 'ma', 'di', 'wo', 'do', 'vr', 'za'];
	var DAYS_OF_WEEK_FR = ['Dim', 'Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam'];
	var MONTHS_EN = [ "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" ];
	var MONTHS_RU = [ "Ð¯Ð½Ð²", "Ð¤ÐµÐ²", "ÐœÐ°Ñ€", "Ð�Ð¿Ñ€", "ÐœÐ°Ð¹", "Ð˜ÑŽÐ½", "Ð˜ÑŽÐ»", "Ð�Ð²Ð³", "Ð¡ÐµÐ½", "ÐžÐºÑ‚", "Ð�Ð¾Ñ�", "Ð”ÐµÐº" ];
	var MONTHS_BR = [ "Janeiro", "Fevereiro", "MarÃ§o", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro" ];
	var MONTHS_PT = [ "janeiro", "fevereiro", "marÃ§o", "abril", "maio", "junho", "julho", "agosto", "setembro", "outubro", "novembro", "dezembro" ];
	var MONTHS_CN = [ "ä¸€æœˆ", "äºŒæœˆ", "ä¸‰æœˆ", "å››æœˆ", "äº”æœˆ", "å…­æœˆ", "ä¸ƒæœˆ", "å…«æœˆ", "ä¹�æœˆ", "å��æœˆ", "å��ä¸€æœˆ", "å��äºŒæœˆ"];
	var MONTHS_DE = [ "Jan", "Feb", "MÃ¤rz", "Apr", "Mai", "Juni", "Juli", "Aug", "Sept", "Okt", "Nov", "Dez" ];
	...
	var MONTHS_TR = [ "Ock", "Åžub", "Mar", "Nis", "May", "Haz", "Tem", "Agu", "Eyl", "Ekm", "Kas", "Arlk" ];
	var MONTHS_ES = [ "ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic" ];
	var MONTHS_KO = [ "1ì›”", "2ì›”", "3ì›”", "4ì›”", "5ì›”", "6ì›”", "7ì›”", "8ì›”", "9ì›”", "10ì›”", "11ì›”", "12ì›”" ];
	...
	var parseDate = function (str, opt_date_format) {
		if(opt_date_format != null){
			...
			var df = opt_date_format.replace(/(-|\/)/g, '[-\/]')
				...
				.replace(/(YY|MM|DD|hh|mm|s)/g, '(\\d{1,2})')
			...
			if( m != null){
				...
				while (df != null && 0 < df.length) {
					...
					if (format_before_c != format_c) {
						if(/(YYYY|YY|MM|DD|mm|dd|M|D|h|m|s)/.test(format_buf)){
				...
				if (format_buf != '' && /(YYYY|YY|MM|DD|mm|dd|M|D|h|m|s)/.test(format_buf)){
					formats.push( format_buf );
				}
	...
	var draw = function($picker, option, year, month, day, hour, min, sec) {
		var date = new Date();
		...
		var $timelist = $picker.children('.datepicker_inner_container').children('#hours');
		var $minslist = $picker.children('.datepicker_inner_container').children('#mins');
		var $secslist = $picker.children('.datepicker_inner_container').children('#secs');
		...
		if (isAnim == true) {
			if(changePoint == "calendar"){
				...
				$minslist.stop().queue([]);
				$minslist.fadeTo("fast", 0.8);
				$secslist.stop().queue([]);
				$secslist.fadeTo("fast", 0.8);
			}
		}
		/* Remind timelist scroll state */
		var drawBefore_timeList_scrollTop = $timelist.scrollTop();
		var drawBefore_minsList_scrollTop = $minslist.scrollTop();
		var drawBefore_secsList_scrollTop = $secslist.scrollTop();

		/* New timelist  */
		var timelist_activeTimeCell_offsetTop = -1;
		var minslist_activeTimeCell_offsetTop = -1;
		var secslist_activeTimeCell_offsetTop = -1;
			...
			if (isFutureOnly && isPast) {
				...
				$td.click(function() {
					...
					draw($picker, {
						"isAnim": false,
						"isOutputToInputObject": true
					}, targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), selectedDate.getHours(), selectedDate.getMinutes(), selectedDate.getSeconds());
				});
		...
		if ($picker.data("dateOnly") == true) {
			...
			$minslist.children().remove();
			$secslist.children().remove();

			/* Set height to Timelist (Calendar innerHeight - Calendar padding) */
			$timelist.css("height", $calendar.innerHeight() - 10 + 'px');
			$minslist.css("height", $calendar.innerHeight() - 10 + 'px');
			$secslist.css("height", $calendar.innerHeight() - 10 + 'px');
			/* Output time cells */
			timelist_activeTimeCell_offsetTop = fillList($timelist, getPickedDate($picker), "h");
			minslist_activeTimeCell_offsetTop = fillList($minslist, getPickedDate($picker), "m");
			secslist_activeTimeCell_offsetTop = fillList($secslist, getPickedDate($picker), "s");
		
			/* Scroll the timelist */
			if(isScroll == true){
				/* Scroll to new active time-cell position */
				$timelist.scrollTop(timelist_activeTimeCell_offsetTop - $timelist.offset().top);
				$minslist.scrollTop(minslist_activeTimeCell_offsetTop - $minslist.offset().top);
				$secslist.scrollTop(secslist_activeTimeCell_offsetTop - $secslist.offset().top);
			}else{
				/* Scroll to position that before redraw. */
				$timelist.scrollTop(drawBefore_timeList_scrollTop);
				$minslist.scrollTop(drawBefore_minsList_scrollTop);
				$secslist.scrollTop(drawBefore_secsList_scrollTop);
			}
		}
	...
	var fillList = function($list, date, opt) {
		var n = 60;
		if (opt == "h") {
			n = 24;
		}
		for (var hour = 0; hour < n; hour++) {
			var $o = $('<div>');			
			$o.addClass('timelist_item');
			$o.text(zpadding(hour));
			if (opt == "h") {
				$o.data("hour", hour);
			} else if (opt == "m") {
				$o.data("min", hour);
			...
			if (hour == date.getHours() && opt == "h") {/* selected time */
				$o.addClass('active');
				timelist_activeTimeCell_offsetTop = $o.offset().top;
			} else if (hour == date.getMinutes() && opt == "m") {
				$o.addClass('active');
				minslist_activeTimeCell_offsetTop = $o.offset().top;
			} else if (hour == date.getSeconds() && opt == "s") {
				$o.addClass('active');
				secslist_activeTimeCell_offsetTop = $o.offset().top;
			}
			...
			$o.click(function() {
				...
				draw($picker, {
					"isAnim": false,
					"isOutputToInputObject": true
				}, date.getFullYear(), date.getMonth(), date.getDate(), (opt == "h") ? hour : date.getHours(), (opt == "m") ? min : date.getMinutes(), (opt == "s") ? sec : date.getSeconds());
		...
		if (opt == "h") {
			return timelist_activeTimeCell_offsetTop;
		} else if (opt == "m") {
			return minslist_activeTimeCell_offsetTop;
		} else if (opt == "s") {
			return secslist_activeTimeCell_offsetTop;
		}
	...
	var init = function($obj, opt) {
		...
		$timelist.attr("id", "hours");
		$inner.append($timelist);
		var $minlist = $('<div>');
		$minlist.addClass('datepicker_timelist');
		$minlist.attr("id", "mins");
		$inner.append($minlist);
		var $seclist = $('<div>');
		$seclist.addClass('datepicker_timelist');
		$seclist.attr("id", "secs");
		$inner.append($seclist);
