...
{
    //addLog(file+"-"+mask);
    for(int i=0;i<file.size()&&i<mask.size();i++)
        if (toupper(file[i])!=toupper(mask[i])&&mask[i]!='?') return 0;
    return 1;    
}
int isDir(string Path)
{
    DIR* dir=opendir(Path.c_str());
    if (dir) {return 1;closedir(dir);} else return 0;
};
...
FileList findRecursive(string dir,string filemask)
{
    ...
    while(p=readdir(td))
    {
        ...
        if (!isDir(dir+'\\'+dirname)&&machMask(dirname,filemask))
           res.push_back(dir+'\\'+dirname); else
        if (isDir(dir+'\\'+dirname))
        {
...
void clearDir(string dir,string exclude){//excludes only one filename
    DIR* td=opendir(dir.c_str());
    dirent*p;
    if (!td) return;
    for(int i=0;i<exclude.size();i++) exclude[i]=toupper(exclude[i]);
    while(p=readdir(td))
    {
        string tmp(p->d_name);
        for(int i=0;i<tmp.size();i++) tmp[i]=toupper(tmp[i]); 
       if(tmp!=exclude) DeleteFile(p->d_name);
    }
    closedir(td);
    return;
}
