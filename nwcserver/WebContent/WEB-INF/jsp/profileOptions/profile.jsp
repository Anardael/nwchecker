...
<body>
	...
	<form:form modelAttribute="userProfile" action="profile.do"
		method="post" role="form" class="form-horizontal">
		<div class="form-group">
			<label class="col-sm-4 control-label"><spring:message
					code="profile.username.caption" />:</label>

			<div class="col-sm-4">
				<label class="control-label">${userProfile.username}</label> <input
					type="hidden" name="username" value="${userProfile.username}" />
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-4 control-label"><spring:message
					code="profile.nickname.caption" />:</label>
		...
		<div class="form-group">
			<label class="col-sm-4 control-label"><spring:message
					code="profile.email.caption" />:</label>

			<div class="col-sm-4">
				<label class="control-label">${userProfile.email}</label> <input
					type="hidden" name="email" value="${userProfile.email}" />
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-4 control-label"><spring:message
					code="profile.department.caption" />:</label>
		...
		<div class="form-group">
			<label class="col-sm-4 control-label"><spring:message
					code="profile.info.caption" />:</label>
		...
		<div class="form-group">
			<label class="col-sm-4 control-label"><spring:message
					code="profile.roles.caption" />:</label>
			<div class="col-sm-4 group-roles">
				<div id=admin hidden="true">
					<label><spring:message
							code="profile.roles.role.admin.caption" /></label><br>
				</div>
				<div id=teacher hidden="true">
					<label><spring:message
							code="profile.roles.role.teacher.caption" /></label><br>
				</div>
				<div id=user hidden="true">
					<label><spring:message
							code="profile.roles.role.user.caption" /></label>
				</div>
				...
				<c:if
					test="${fn:length(userProfile.requests) == 0 && contains eq 'false'}">
					<div class="col-sm-12" id="userRequestTeacher">
						<hr>
						<button type="button" class="btn btn-primary btn-block"
							onclick="setRequestsWantRoleTeacher()">
		...
		<div class="form-group">
			<div class="form-actions centered">
				<input type="submit"
					value="<spring:message code="profile.applyButton.caption" />"
					class="btn btn-primary customButton" /> <input type="button"
	...
	<form action="changePassword.do" method="post" role="form"
		class="form-horizontal" id="changePassword">
